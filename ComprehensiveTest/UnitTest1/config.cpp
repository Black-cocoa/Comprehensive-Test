...
message_t *get_case_space()
{
	...
	sectioninfo->max = GetPrivateProfileIntA("message", "max", 20, CONFIG_INI);
	sectioninfo->name = GetPrivateProfileIntA("message", "name", 30, CONFIG_INI);
	return sectioninfo;
}
...
int analyze(char* sectionaddr, int *paraandresult)//分析字符串返回相关参数和结果
{
	...
	GetPrivateProfileStringA(sectionaddr, "key", "", data, sizeof(data), CONFIG_INI);//得到有效字符串
	...
	paraandresult[j] = (int)GetPrivateProfileIntA(sectionaddr, "value", 30, CONFIG_INI);//获取预期结果存入
	return 0;
}